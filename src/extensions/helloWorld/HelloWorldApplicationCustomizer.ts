...
export default class HelloWorldApplicationCustomizer
  ...
  public onInit(): Promise<void> {
    ...
    if (check.indexOf('/') != -1) {
      ...
      a = b.substring(0, b.indexOf('/') + 1);
      b = b.replace(a, '');
      webServerRelativeUrl = '/' + a + b.substring(0, b.indexOf('/'));
    }
    ...
    setInterval(function () {
      ...
      for (var i = 0; i < imgs.length; i++) {
        if (((imgs[i].title.indexOf('mmap File') + 1) || ((imgs[i].classList[0] == 'FileTypeIcon-icon') && (imgs[i].parentElement.title.indexOf('mmap File') + 1))) && (imgs[i].src.indexOf('genericfile.png') == imgs[i].src.length - 15)) {
